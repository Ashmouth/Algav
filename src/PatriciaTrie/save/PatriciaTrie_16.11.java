        ...
        if ((word.length() == 0) && (ptchilds.containsKey(endword) == true)) {
            ...
            for (String key : ptchilds.keySet()) {
            	if (word.startsWith(key)) {
            		...
            		//System.out.println("key = "+key+" word = "+word+ " suffix " + suffix);
            		ptchilds.remove(key);
            		ptchilds.put(key, newkey);
            		ptree.setChilds(ptchilds);
            		return ptree;
            	}
            	System.out.println("Voici la clé = "+key+" cherchant = "+word);
